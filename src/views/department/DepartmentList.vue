  ...
  <page-header-wrapper>
    <a-card :bordered="false">
      <div class="table-operator">
        <a-button v-if="selectedRowKeys.length > 0" icon="minus" @click="handleDelete">删除</a-button>
      </div>
      <s-table
        ref="table"
        size="default"
      ...
      >
        <span slot="action" slot-scope="text, record">
          <template>
            <a @click="handleAdd(record)"><a-icon type="plus" /> 添加</a>
  ...
  data () {
    ...
    return {
      ...
      loadData: parameter => {
        ...
        return getDepartmentList(requestParameters)
          .then(res => {
            return res.result
          })
      },
